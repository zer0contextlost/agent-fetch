...
async function runOnPage(context, url, options) {
  ...
  try {
    ...
    const getOutput = async () => {
      if (recipe) return recipe.extract(page, { limit });
    ...
    try {
      ...
      // Some sites (e.g. unicode.org, found via stress-testing this tool)
      // keep navigating client-side after the network 'load' event fires —
      // if extraction lands mid-navigation, the execution context gets
      // torn down under us. Give the page one more moment to settle and
      // retry once rather than failing on what's really just bad timing.
      if (/execution context was destroyed|cannot read propert.*of null/i.test(err.message)) {
...
module.exports = { fetchPage, fetchMany, recipes, runOnPage, openContext };
